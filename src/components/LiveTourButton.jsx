...
export default function LiveTourButton({ property }) {
  ...
  const startCall = useCallback(async () => {
    ...
    // Production: auto-close if agent doesn't join within 90 seconds
    setTimeout(() => {
      setScreen(prev => {
        if (prev === SCREEN.CALL) {
          // Still in call — check participant count
          try {
            const count = jitsiApiRef.current?.getNumberOfParticipants?.() || 1;
            if (count <= 1) {
